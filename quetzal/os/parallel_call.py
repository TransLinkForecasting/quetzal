...
def parallel_call_subprocess(
    subprocess_filepath, kwarg_list, dump_kwargs=json.dump, load_result=json.load, leave=False, *args, **kwargs
):
    input_files = []
    output_files = []
    uids = []
...
def parallel_executor(function, method='pool', num_workers: int = 1, parallel_kwargs: dict = {}, **kwargs):
    ...
    if method == 'pool':
        results = process_pool_executor(
            function=function, num_workers=num_workers, parallel_kwargs=parallel_kwargs, **kwargs
        )

    elif method == 'pipe':
        results = process_pipe_executor(
            function=function, num_workers=num_workers, parallel_kwargs=parallel_kwargs, **kwargs
        )

    elif method == 'queue':
        results = process_queue_executor(
            function=function, num_workers=num_workers, parallel_kwargs=parallel_kwargs, **kwargs
        )

    elif method == 'manager':
        results = process_manager_executor(
            function=function, num_workers=num_workers, parallel_kwargs=parallel_kwargs, **kwargs
        )
...
def process_pool_executor(function, num_workers=1, parallel_kwargs={}, **kwargs):
    ...
    with ProcessPoolExecutor(max_workers=num_workers) as executor:
        for i in range(num_workers):
            ...
            p = executor.submit(function, **batch_kwargs, **kwargs)
            results.append(p)
